...
class MultiSelectComboBox(QComboBox):
    ...
    def updateText(
        ...
        elidedText = metrics.elidedText(
            self.placeholderText, Qt.TextElideMode.ElideRight, self.lineEdit().width()
        )
    ...
    def currentData(
        ...
        return [self.typeSelection(i, output_type) for i in range(self.model().rowCount()) if
                self.model().item(i).checkState() == Qt.CheckState.Checked]

    def setCurrentIndexes(
            self,
            indexes: list
    ) -> None:
