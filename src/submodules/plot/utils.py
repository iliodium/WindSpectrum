...
def calculate_levels_pressure(parameter, pressure_coefficients):
    min_value = calculate_function_from_pressure_coefficients(pressure_coefficients, np.min, 0)
    max_value = calculate_function_from_pressure_coefficients(pressure_coefficients, np.max, 0)
    ...
    levels = np.round(np.arange(min_value - step, max_value + step, step), 0)
